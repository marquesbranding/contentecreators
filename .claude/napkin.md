...
8. **[2026-07-23] Make required-field behavior consistent and accessible**
   Do instead: use the shared red required indicator and form notice, preserve native `required` semantics, validate native and custom shadcn/Base UI controls on first blur and while correcting through `useRequiredFieldValidation`, keep submit disabled until the complete form is valid, expose minimum lengths beside their fields, compare password confirmation before submit, keep a visible 44 px show/hide control on mobile, show adjacent `pt-BR` errors with `aria-invalid`, and focus the first invalid field before dispatching a Server Action. Dispatch recoverable form actions manually inside a transition so React does not reset uncontrolled values when the server returns a validation error.
